...
(use-package org
  ...
  (custom-set-faces
   '(org-level-1 ((t (:bold t  :height 1.1 :family "Helvetica Neue"))))
...
(use-package elpy
  ...
  (elpy-enable)
  (elpy-use-ipython)
  (setq elpy-rpc-backend "jedi")       ;use jedi for auto completion
)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :rangle no
...
(use-package neotree
  ...
  (setq neo-theme 'nerd)
  (setq neo-window-fixed-size nil)) ; 'classic, 'nerd, 'ascii, 'arrow
#+END_SRC
